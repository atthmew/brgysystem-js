...
exports.renderBorrowRecord = async (req, res) => {
	try {
		...
		const borrower = await Borrower.find();
		res.render('user_pages/borrow_pages/borrowers_record', {
			title: "User Borrowers' Record",
			borrower: borrower,
			// borrower: borrower,
		});
	} catch (err) {
...
exports.renderLendMaterial = async (req, res) => {
	try {
		...
		res.render('user_pages/borrow_pages/lend_material', {
			title: 'User Lend Material Page',
			borrower: borrower,
			user: user,
		});
	} catch (err) {
...
exports.lendMaterial = async (req, res) => {
	try {
		...
		res.redirect('/user/borrower_record');
	} catch (err) {
		if (err) {
			console.log(err);
			req.session.message = {
...
exports.markAsReturnedConfirmation = async (req, res) => {
	try {
		...
		res.render('user_pages/borrow_pages/returnedConfirmation', {
...
exports.renderMoreInfoBorrow = async (req, res) => {
	try {
		...
		res.render('user_pages/borrow_pages/more_info_borrow', {
			title: 'User More Info about Borrower Page',
			borrow: borrow,
		});
	} catch (err) {
		if (err) {
